...
**Лабораторная работа №2: Реализация ETL-процесса с использованием Apache Spark**
...
2. Скопируйте токен Jupyter Notebook из логов Docker
3. Откройте веб-интерфейс Jupyter Lab по адресу [localhost:8890](http://localhost:8890/)
4. Введите токен и перейдите в директорию `work`
5. Используйте ноутбук `build_fill_star.ipynb` для построения модели «звезда»
6. Используйте ноутбук `build_reports_ch.ipynb` для создания витрин в ClickHouse

---

## Описание задачи

Одним из самых популярных фреймворков для работы с Big Data является Apache Spark — мощный инструмент, предоставляющий широкий набор функций для создания ETL-пайплайнов.

**Задание:** реализовать ETL-пайплайн на Apache Spark, который:
1. Преобразует исходные данные (файлы `mock_data.csv`) в модель данных «звезда» в PostgreSQL
2. На основе модели «звезда» создаёт аналитические отчёты в NoSQL базах данных
