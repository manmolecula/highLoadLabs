...
Если требуется запускать скрипты отдельно, то:
```bash
BigDataTrino-main> docker-compose up
```
И затем:
```bash
BigDataTrino-main> docker exec -it trino trino
```

Далее требуется поочередно вставлять содержимое sql-скриптов из папки **trino_scripts**.

Пример SQL-запроса к витрине среднего чека в магазинах:
```sql
SELECT (*) FROM default.mart_avg_check_store ORDER BY avg_check DESC LIMIT 5 ;
```
![Результат sql-запроса](static/result.png)
## Задание
